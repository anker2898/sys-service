...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 11-04-2023 a las 03:51:51
...
CREATE OR REPLACE PROCEDURE
 `SP_AUTH` (IN `P_SUSER` VARCHAR(80), IN `P_SPASSWORD` VARCHAR(255))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_AUTH_ROLES` (IN `P_SUSER` VARCHAR(80), IN `P_SPASSWORD` VARCHAR(80))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_CAMBIO_ESTADO_RECIBO` (IN `P_FLOW` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_DEL_CLIENT` (IN `P_NID` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DEL_USER` (IN `P_SDOCUMENT` VARCHAR(9))  DETERMINISTIC BEGIN
    ...
    DELETE FROM usuario_privilegio
        WHERE NUSER=V_NUSER;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INS_CASAS` (IN `P_SDOCUMENT` VARCHAR(8), IN `P_SEMAIL` VARCHAR(100), IN `P_SNUMERO` VARCHAR(9), IN `P_NCONDOMINIO` INT, IN `P_SMANZANA` VARCHAR(2), IN `P_NLOTE` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_CLIENT` (IN `P_SDOCUMENT` VARCHAR(11), IN `P_SNOMBRE` VARCHAR(99), IN `P_SAPELLIDO_PAT` VARCHAR(99), IN `P_SAPELLIDO_MAT` VARCHAR(99), IN `P_SEMAIL` VARCHAR(99), IN `P_SNUMBER` VARCHAR(11), IN `P_SDIRECCION` VARCHAR(99), IN `P_NDEPARTAMENTO` VARCHAR(2), IN `P_NPROVINCIA` VARCHAR(4), IN `P_NDISTRITO` VARCHAR(6))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_MEDICION` (IN `P_NID` INT, IN `P_NMEDICION` DECIMAL(20,4), IN `P_SUSER` VARCHAR(100), IN `P_DREGISTRO` DATE, IN `P_SIMAGEN` VARCHAR(200))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_PAGO` (IN `P_NID` INT, IN `P_NPAGO` DECIMAL(8,2), IN `P_SUSUARIO` VARCHAR(50))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_PARAMETER` (IN `P_SLABEL` VARCHAR(100), IN `P_SVALUE` VARCHAR(250), IN `P_CTYPE` VARCHAR(1))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_RECIBOS` (IN `P_NFLOW` INT, IN `P_DEMISION` DATE, IN `P_DVENCIMIENTO` DATE, IN `P_DVENC_ULTIMO` DATE)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_SERVICIOS` (IN `P_NIDCASA` INT, IN `P_NIDSERVICIO` INT, IN `P_DINICIO_SERVICIO` DATE, IN `P_NSTATUS` INT, IN `P_NMEDIDA` DECIMAL(20,4))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_USER` (IN `P_SDOCUMENT` VARCHAR(8), IN `P_SNOMBRE` VARCHAR(100), IN `P_SAPELLIDO_PAT` VARCHAR(100), IN `P_SAPELLIDO_MAT` VARCHAR(100), IN `P_SEMAIL` VARCHAR(100), IN `P_SNUMBER` VARCHAR(9), IN `P_NSTATUS` INT, IN `P_SUSER` VARCHAR(50), IN `P_NPASSWORD_RESET` INT, IN `P_SDIRECCION` VARCHAR(250), IN `P_SPASSWORD` VARCHAR(250))   BEGIN
	...
	IF COUNT_USER = 0 THEN
		...
		VALUES(TRIM(P_SDOCUMENT), TRIM(P_SNOMBRE), TRIM(P_SAPELLIDO_PAT), TRIM(P_SAPELLIDO_MAT), TRIM(P_SEMAIL), TRIM(P_SNUMBER), P_NSTATUS, TRIM(P_SPASSWORD), TRIM(P_SDIRECCION), TRIM(P_SUSER));
	ELSE 
		UPDATE usuario SET 
			SNOMBRE = TRIM(P_SNOMBRE), 
			SAPELLIDO_PAT = TRIM(P_SAPELLIDO_PAT), 
			SAPELLIDO_MAT = TRIM(P_SAPELLIDO_MAT), 
			SEMAIL = TRIM(P_SEMAIL), 
			SNUMBER = TRIM(P_SNUMBER),
...
CREATE OR REPLACE PROCEDURE
 `SP_INS_USER_PRIVILEGIOS` (IN `P_SDOCUMENT` VARCHAR(80), IN `P_SPRIVILEGIOS` VARCHAR(80), IN `P_BSTATUS` INT)   BEGIN
	DECLARE NIDUSER INT;
...
CREATE OR REPLACE PROCEDURE
 `SP_RESET_PASSWORD` (IN `P_SUSER` VARCHAR(80), IN `P_SPASSWORD` VARCHAR(80))   BEGIN
	UPDATE usuario 
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_CASA_ID` (IN `P_NID` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_CLIENT_DNI` (IN `P_NID` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_CLIENT_GENERAL_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_DISTRITO` (IN `P_NPROVINCIA` VARCHAR(4))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_PARAMETER_ID` (IN `P_NID` INT)   BEGIN
	SELECT SLABEL AS "LABEL",
            SVALUE AS "VALOR",
            CTYPE  AS "TIPO",
            NID    AS "ID"
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_PROVINCIA` (IN `P_NDEPARTAMENTO` VARCHAR(2))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_RECIBO` (IN `P_NRECIBO` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_ROLES_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN
	SELECT P.SLABEL   AS "LABEL",
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_SERVICIOS_CASA` (IN `P_NIDCASA` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_SERVICIO_RECIBO` (IN `P_NSUMINISTRO` DECIMAL(11), IN `P_NSERVICIO` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_URBANIZACION` ()   BEGIN

	SELECT NID  AS "ID",
           SURBANIZACION AS "CONDOMINIO"
    FROM urbanizaciones;

END$$

CREATE OR REPLACE PROCEDURE
 `SP_SEL_URBANIZACION_SERVICIOS` (IN `P_NCONDOMINIO` INT, IN `P_NSERVICIO` INT)   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_USER` (IN `P_SUSER` VARCHAR(80))   BEGIN
	SELECT U.SUSER    AS "USUARIO",
		   U.NSTATUS  AS "ESTADO",
		   U.SNOMBRE AS "NOMBRE",
		   U.SAPELLIDO_PAT AS "APELLIDO_PAT",
...
CREATE OR REPLACE PROCEDURE
 `SP_SEL_USER_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_VAL_CLIENT_DNI` (IN `P_SDOCUMENT` VARCHAR(11))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_VAL_PARAMETER` (IN `P_SLABEL` VARCHAR(99))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_VAL_SUMINISTRO` (IN `P_NSUMINISTRO` DECIMAL(11))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_VAL_USER` (IN `P_SUSER` VARCHAR(50))   BEGIN
...
CREATE OR REPLACE PROCEDURE
 `SP_VAL_USER_DOCUMENT` (IN `P_SDOCUMENT` VARCHAR(8))   BEGIN
...
(2, 'AAAAAAAAAAA', 'AAAAAA', 'AAAAAAAAAAAAAAA', 'AAAAAAAAAAAAAAA', 'AAAAAAAAAAAAAAA@ASD.COM', 'AAAAAAAAAAA', 'SADSADASD', '15', '1501', '150142', 1),
(3, 'P_SDOCUMENT', 'Anker', 'QQQQQQQQQQQQW', 'WWWWWWWWE', 'asd@asd.com', 'EWQ', 'QWEEQWEQW', '11', '1102', '110202', 1),
(4, '12345678', 'Anker', 'Gutierrez', 'Huamanttupa', 'ASD@ASD.COM', '987654321', 'Villa el salvador', '10', '1002', '100203', 1),
(5, '78946546', 'AAAAA', 'AAAA', 'AAAAA', 'ASD@ASD.COM', '89789897879', 'dfssdffsdfsdasdsad af', '16', '1602', '160205', 1);
...
(2, 'AAAAAAS', 'qweqweeqwasdasdasdasdasdasd', 'S'),
(7, 'Agua', '1.562', 'N'),
(8, 'AAAAAAS1', 'asfdfsadsadasd', 'S'),
(10, 'Luz 1', '5', 'N'),
(11, 'Luz 2', '6', 'N'),
(12, 'asasasdsda', '465.012', 'N');
...
(7, 0, 'aadsasfdsgfdsgfd', '2559755edd4cda058dc6434c5f168d16', 0, '98876546', 'Guillermo', 'AAAA', 'AAAAA', '79879sdas', 'Surco Nuevo', 'ASD@ASD.COM'),
(8, 1, 'carlos', 'e2525a79f92782b3f3e32fc10ce7e5b1', 0, '11323215', 'Carlos', 'Raymond', 'Julian', '123246549', 'Av. 3 de mayo', 'hola@gmail.com'),
(10, 1, 'carlos', '076a470107d68cd662525122e0e667c1', 1, '1132321a', 'Carlos123', 'Raymond123', 'Julian123', '12324654a', 'Av. 3 de mayo', 'hola@gmail.com'),
(11, 1, 'Test2', 'a88ebe65a1239970da4aaf532d326cc6', 0, '5214783a', 'Testabc', 'Test1abc', 'Test2abc', '54685c8ab', '21214121abc', 'test2@gmail.com');
