...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-05-2023 a las 07:10:32
...
CREATE PROCEDURE `SP_DEL_PARAMETER` (IN `P_NID` INT)   BEGIN
...
CREATE PROCEDURE `SP_DEL_USER` (IN `P_SDOCUMENT` VARCHAR(9))  DETERMINISTIC BEGIN
...
CREATE PROCEDURE `SP_INS_CASAS` (IN `P_SDOCUMENT` VARCHAR(8), IN `P_SEMAIL` VARCHAR(100), IN `P_SNUMERO` VARCHAR(9), IN `P_NCONDOMINIO` INT, IN `P_SMANZANA` VARCHAR(2), IN `P_NLOTE` INT, IN `P_NSUMINISTRO` INT)   BEGIN
    ...
    DECLARE V_NCASA   INT;
    DECLARE V_NIDCASA INT;
    
    SELECT C.NID
    INTO V_NCLIENT
    FROM clientes C
    WHERE C.SDOCUMENT = 	P_SDOCUMENT;
    
    UPDATE clientes C
    SET C.SEMAIL = P_SEMAIL,
        C.SNUMBER = P_SNUMERO,
        C.NSTATUS = 1
    WHERE C.NID = V_NCLIENT;
    
    SELECT COUNT(*)
    INTO V_NCASA
    FROM casas C
    WHERE C.NCLIENTE = V_NCLIENT 
    AND C.NURBANIZACION = P_NCONDOMINIO
    AND C.SMANZANA = P_SMANZANA
    AND C.NLOTE = P_NLOTE;
    ...
    SELECT V_NIDCASA AS "ID"
    FROM DUAL;
	
    COMMIT;
END$$

CREATE PROCEDURE `SP_INS_CLIENT` (IN `P_SDOCUMENT` VARCHAR(11), IN `P_SNOMBRE` VARCHAR(99), IN `P_SAPELLIDO_PAT` VARCHAR(99), IN `P_SAPELLIDO_MAT` VARCHAR(99), IN `P_SEMAIL` VARCHAR(99), IN `P_SNUMBER` VARCHAR(11), IN `P_SDIRECCION` VARCHAR(99), IN `P_NDEPARTAMENTO` VARCHAR(2), IN `P_NPROVINCIA` VARCHAR(4), IN `P_NDISTRITO` VARCHAR(6))   BEGIN
    ...
    IF COUNT_CLIENT > 0 THEN
    	...
    	UPDATE clientes
        SET SNOMBRE = TRIM(P_SNOMBRE),
            SAPELLIDO_PAT = TRIM(P_SAPELLIDO_PAT),
            SAPELLIDO_MAT = TRIM(P_SAPELLIDO_MAT),
            SEMAIL = TRIM(P_SEMAIL),
            SNUMBER = TRIM(P_SNUMBER),
            SDIRECCION = TRIM(P_SDIRECCION),
            NDEPARTAMENTO = TRIM(P_NDEPARTAMENTO),
            NPROVINCIA = TRIM(P_NPROVINCIA),
            NDISTRITO = TRIM(P_NDISTRITO),
            NSTATUS = 1
         WHERE SDOCUMENT = TRIM(P_SDOCUMENT);

    ELSE
    	INSERT INTO clientes (SDOCUMENT, SNOMBRE, SAPELLIDO_PAT, SAPELLIDO_MAT, SEMAIL, SNUMBER, SDIRECCION, NDEPARTAMENTO, NPROVINCIA, NDISTRITO)
        VALUES (TRIM(P_SDOCUMENT), TRIM(P_SNOMBRE), TRIM(P_SAPELLIDO_PAT), TRIM(P_SAPELLIDO_MAT), TRIM(P_SEMAIL), TRIM(P_SNUMBER), TRIM(P_SDIRECCION), TRIM(P_NDEPARTAMENTO), TRIM(P_NPROVINCIA), TRIM(P_NDISTRITO));
...
CREATE PROCEDURE `SP_INS_MEDICION` (IN `P_NID` INT, IN `P_NMEDICION` DECIMAL(20,4), IN `P_SUSER` VARCHAR(100), IN `P_DREGISTRO` DATE, IN `P_SIMAGEN` VARCHAR(200))   BEGIN
  ...
  DECLARE V_NMEDICION_MIN	DECIMAL(20,4);
  DECLARE V_NMEDICION		DECIMAL(20,4);
  DECLARE V_NMONTO_MIN		DECIMAL(8,2);
  DECLARE V_NMONTO 			DECIMAL(8,2);
  DECLARE V_NDEUDA 			DECIMAL(8,2);
  DECLARE V_NSUBTOTAL 		DECIMAL(8,2);
  DECLARE V_NTOTAL 			DECIMAL(8,2);
  
  #OBTENER ID DEL USUARIO
  SELECT U.NID INTO V_NIDUSER
    FROM usuario U
   WHERE U.SUSER = P_SUSER;
  
  #OBTENER ID DEL SERVICIO
  SELECT CS.NSERVICIO INTO V_NIDSERVICIO
    FROM detalle_pago P
   INNER JOIN casas_servicios CS ON CS.NID = P.NSERVICIO
   WHERE P.NID = P_NID;
  
  #OBTENER PRECIO DE SERVICIOS
  IF V_NIDSERVICIO = 1 THEN #LUZ
  
   SELECT CONVERT(PS.SVALUE, DECIMAL(5,2))
     INTO V_NVALOR
   ...
   SELECT CONVERT(PS.SVALUE, DECIMAL(20,4))
     INTO V_NMEDICION_MIN
     FROM parametros_servicios PS
    WHERE PS.SLABEL = 'MEDICION_MIN_LUZ';
   
   SELECT CONVERT(PS.SVALUE, DECIMAL(8,2))
     INTO V_NMONTO_MIN
     FROM parametros_servicios PS
    WHERE PS.SLABEL = 'PAGO_MIN_LUZ';
    
  ELSEIF V_NIDSERVICIO = 2 THEN #AGUA
  ...
  IF V_NIDSERVICIO = 1 THEN 
     ...
     IF V_NMEDICION_MIN IS NULL THEN
        SET V_NMEDICION_MIN = 0.0000;
      END IF;
     
     IF V_NMONTO_MIN IS NULL THEN
        SET V_NMONTO_MIN = 0.00;
     END IF;
     
     SELECT NMEDICION_INI, NDEUDA 
       INTO V_NMEDICION, V_NDEUDA
       FROM detalle_pago
      WHERE NID = P_NID; 
     
     #VALIDACION DEL MONTO MINIMO    
     IF (P_NMEDICION - V_NMEDICION) > V_NMEDICION_MIN THEN
     	SET V_NMONTO = V_NVALOR * (P_NMEDICION - V_NMEDICION);
     ELSE
     	SET V_NMONTO = V_NMONTO_MIN;
     END IF;
    
     SET V_NSUBTOTAL = (V_NVALOR * (P_NMEDICION - DP.NMEDICION_INI)) + V_NMANTENIMIENTO + V_NALUMBRADO;
     SET V_NTOTAL    = V_NDEUDA + NSUBTOTAL;
  ...
  ELSEIF V_NIDSERVICIO = 2 THEN 
     ...
     SELECT NDEUDA 
      ...
      WHERE NID = P_NID;  
  	 ...
  	 SET V_NMONTO    = V_NVALOR * P_NMEDICION;
     SET V_NSUBTOTAL = NMONTO;
     SET V_NTOTAL    = V_NDEUDA + NSUBTOTAL;
...
CREATE PROCEDURE `SP_INS_PARAMETER` (IN `P_SLABEL` VARCHAR(100), IN `P_SVALUE` VARCHAR(250), IN `P_CTYPE` VARCHAR(1), IN `P_NIDUSER` INT)   BEGIN
	DECLARE COUNT_PARAMETER INT;
	DECLARE V_NPARAMETRO	INT; 
    
    SELECT COUNT(*)
      INTO COUNT_PARAMETER
      FROM parametros_servicios
     WHERE SLABEL = TRIM(P_SLABEL);
    
    IF COUNT_PARAMETER = 0 then
    
    	INSERT INTO parametros_servicios (SLABEL, SVALUE, CTYPE)
        VALUES (TRIM(P_SLABEL), TRIM(P_SVALUE), P_CTYPE);
       
    ELSE
    
    	UPDATE parametros_servicios
        SET SVALUE = TRIM(P_SVALUE)
		WHERE SLABEL = TRIM(P_SLABEL);
    ...
    INSERT INTO parametros_servicios_log (NIDUSER, NPARAMETRO, SVALUE, DUPDATE)
    VALUES (P_NIDUSER, V_NPARAMETRO, TRIM(P_SVALUE), SYSDATE());
...
CREATE PROCEDURE `SP_INS_SERVICIOS` (IN `P_NIDCASA` INT, IN `P_NIDSERVICIO` INT, IN `P_DINICIO_SERVICIO` DATE, IN `P_NSTATUS` INT, IN `P_NMEDIDA` DECIMAL(20,4))   BEGIN
    ...
    SELECT COUNT(*)
    INTO V_SERVICIO
    FROM casas_servicios C
    WHERE C.NCASA = P_NIDCASA
    AND C.NSERVICIO = P_NIDSERVICIO;
    
    IF V_SERVICIO = 0 THEN
    
    	INSERT INTO casas_servicios (NCASA, NSERVICIO, DINICIO_SERVICIO, NSTATUS, NMEDIDA)
        VALUES (P_NIDCASA, P_NIDSERVICIO, P_DINICIO_SERVICIO, P_NSTATUS, P_NMEDIDA);
    
    ELSE 

		UPDATE casas_servicios C
        SET C.NSTATUS = P_NSTATUS,
        	C.DINICIO_SERVICIO = P_DINICIO_SERVICIO,
            C.NMEDIDA = P_NMEDIDA
        WHERE C.NCASA = P_NIDCASA
    	AND C.NSERVICIO = P_NIDSERVICIO;
...
CREATE PROCEDURE `SP_INS_USER` (IN `P_SDOCUMENT` VARCHAR(8), IN `P_SNOMBRE` VARCHAR(100), IN `P_SAPELLIDO_PAT` VARCHAR(100), IN `P_SAPELLIDO_MAT` VARCHAR(100), IN `P_SEMAIL` VARCHAR(100), IN `P_SNUMBER` VARCHAR(9), IN `P_NSTATUS` INT, IN `P_SUSER` VARCHAR(50), IN `P_NPASSWORD_RESET` INT, IN `P_SDIRECCION` VARCHAR(250), IN `P_SPASSWORD` VARCHAR(250), IN `P_NURBANIZACION` INT, IN `P_NDEPARTAMENTO` VARCHAR(2), IN `P_NPROVINCIA` VARCHAR(4), IN `P_NDISTRITO` VARCHAR(6))   BEGIN
	...
	IF COUNT_USER = 0 then
		...
		INSERT INTO usuario
		(SDOCUMENT, SNOMBRE, SAPELLIDO_PAT, SAPELLIDO_MAT, SEMAIL, SNUMBER, NSTATUS, SPASSWORD, SDIRECCION, SUSER, NDEPARTAMENTO, NPROVINCIA, NDISTRITO, NACTIVO)
		VALUES(TRIM(P_SDOCUMENT), TRIM(P_SNOMBRE), TRIM(P_SAPELLIDO_PAT), TRIM(P_SAPELLIDO_MAT), TRIM(P_SEMAIL), TRIM(P_SNUMBER), P_NSTATUS, TRIM(P_SPASSWORD), TRIM(P_SDIRECCION), TRIM(P_SUSER), P_NDEPARTAMENTO, P_NPROVINCIA, P_NDISTRITO, 1);
		...
		INSERT INTO usuario_urbanizacion (NUSUARIO, NURBANIZACION)
			VALUES (V_NIDUSER, P_NURBANIZACION);
	...
	ELSE 
		...
		SET	SNOMBRE 		= TRIM(P_SNOMBRE), 
            ...
            NSTATUS 		= P_NSTATUS,
            SDIRECCION 		= TRIM(P_SDIRECCION),
            NPASSWORD_RESET = P_NPASSWORD_RESET,
            NDEPARTAMENTO 	= P_NDEPARTAMENTO, 
            NPROVINCIA  	= P_NPROVINCIA, 
            NDISTRITO 		= P_NDISTRITO,
            NACTIVO 		= 1
		WHERE SDOCUMENT 	= TRIM(P_SDOCUMENT);
		...
		UPDATE usuario_urbanizacion
		   SET NURBANIZACION  = P_NURBANIZACION
		 WHERE NUSUARIO = V_NIDUSER;
...
CREATE PROCEDURE `SP_SEL_CASAS` (IN `P_NIDUSER` INT)   BEGIN

	SELECT C.NID 			AS "ID",
    	   CL.SDOCUMENT 	AS "DOCUMENTO",
           CL.SNOMBRE   	AS "NOMBRE",
           CL.SAPELLIDO_PAT AS "APPATERNO",
           C.NSUMINISTRO	AS "SUMINISTRO",
           U.SURBANIZACION  AS "CONDOMINIO",
           C.SMANZANA		AS "MANZANA",
           C.NLOTE			AS "LOTE"
      FROM casas C
     INNER JOIN urbanizaciones U ON C.NURBANIZACION = U.NID
     INNER JOIN clientes CL ON CL.NID = C.NCLIENTE
     WHERE C.NURBANIZACION IN (SELECT UU.NURBANIZACION  
	  							 FROM usuario_urbanizacion UU
	  							WHERE UU.NUSUARIO = P_NIDUSER);

END$$

CREATE PROCEDURE `SP_SEL_CASA_ID` (IN `P_NID` INT)   BEGIN

	SELECT C.SMANZANA AS "MANZANA",
           C.NLOTE AS "LOTE",
           CL.SDOCUMENT AS "DOCUMENTO",
...
CREATE PROCEDURE `SP_SEL_CLIENT` ()   BEGIN

	SELECT SNOMBRE       AS "NOMBRE",
    	   SAPELLIDO_PAT AS "PATERNO",
           SAPELLIDO_MAT AS "MATERNO",
           SEMAIL		 AS "CORREO",
           SNUMBER		 AS "TELEFONO",
           SDOCUMENT     AS "DOCUMENTO",
           NID			 AS "ID"
     FROM clientes
     WHERE NSTATUS = 1;
...
CREATE PROCEDURE `SP_SEL_CLIENT_DNI` (IN `P_NID` INT)   BEGIN
	SELECT C.SDOCUMENT AS "DOCUMENTO", 
	       C.SNOMBRE AS "NOMBRE", 
	       C.SAPELLIDO_PAT AS "APELLIDO_PAT", 
	       C.SAPELLIDO_MAT AS "APELLIDO_MAT", 
	       C.SEMAIL AS "EMAIL", 
	       C.SNUMBER AS "NUMERO", 
	       C.SDIRECCION AS "DIRECCION",
           C.NDEPARTAMENTO AS "DEPARTAMENTO",
           C.NPROVINCIA AS "PROVINCIA",
           C.NDISTRITO AS "DISTRITO"
	FROM  clientes C
	WHERE C.NID= P_NID;
END$$

CREATE PROCEDURE `SP_SEL_CLIENT_GENERAL_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN

	SELECT C.SDOCUMENT     AS "DOCUMENTO",
           C.SNOMBRE       AS "NOMBRE",
           C.SAPELLIDO_PAT AS "PATERNO",
           C.SAPELLIDO_MAT AS "MATERNO",
           C.SEMAIL	       AS "CORREO",
           C.SNUMBER       AS "TELEFONO"
    FROM clientes C
    WHERE C.SDOCUMENT = P_SDOCUMENT 
    AND C.NSTATUS = 1;
...
CREATE PROCEDURE `SP_SEL_DEPARTAMENTO` ()   BEGIN
	
    SELECT NID           AS "ID",
    	   SDEPARTAMENTO AS "LABEL"
    FROM departamento;

END$$

CREATE PROCEDURE `SP_SEL_DISTRITO` (IN `P_NPROVINCIA` VARCHAR(4))   BEGIN

	SELECT NID AS "ID",
           SDISTRITO AS "LABEL"
    FROM distrito
    WHERE NPROVINCIA = P_NPROVINCIA;

END$$

CREATE PROCEDURE `SP_SEL_PARAMETER` ()   BEGIN
...
CREATE PROCEDURE `SP_SEL_PARAMETER_ID` (IN `P_NID` INT)   BEGIN
	SELECT SLABEL AS "LABEL",
            SVALUE AS "VALOR",
            CTYPE  AS "TIPO",
            NID    AS "ID"
...
CREATE PROCEDURE `SP_SEL_PROVINCIA` (IN `P_NDEPARTAMENTO` VARCHAR(2))   BEGIN

	SELECT NID		  AS "ID",
    	   SPROVINCIA AS "LABEL"
    FROM provincia
    WHERE NDEPARTAMENTO = P_NDEPARTAMENTO;
...
CREATE PROCEDURE `SP_SEL_ROLES_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN
	SELECT P.SLABEL   AS "LABEL",
...
CREATE PROCEDURE `SP_SEL_SERVICIOS` ()   BEGIN
...
CREATE PROCEDURE `SP_SEL_SERVICIOS_CASA` (IN `P_NIDCASA` INT)   BEGIN

	SELECT C.NID AS "ID",
    	   C.NCASA AS "CASA",
           C.NSERVICIO AS "SERVICIO",
           C.NSTATUS AS "STATUS",
           C.DINICIO_SERVICIO AS "INICIO",
           C.NMEDIDA AS "MEDIDA"
    FROM casas_servicios C
    WHERE C.NCASA = P_NIDCASA;
...
CREATE PROCEDURE `SP_SEL_URBANIZACION` (IN `P_NIDUSER` INT)   BEGIN

	SELECT U.NID  	         AS "ID",
           U.SURBANIZACION   AS "CONDOMINIO"
      FROM urbanizaciones U
     INNER JOIN usuario_urbanizacion C ON C.NURBANIZACION = U.NID
     WHERE C.NUSUARIO  = P_NIDUSER 
        OR C.NUSUARIO = 0;

END$$

CREATE PROCEDURE `SP_SEL_URBANIZACION_SERVICIOS` (IN `P_NCONDOMINIO` INT, IN `P_NSERVICIO` INT)   BEGIN

	SELECT CONCAT(CL.SNOMBRE, ' ', CL.SAPELLIDO_PAT, ' ', CL.SAPELLIDO_MAT) AS "TITULAR",
           CL.SDOCUMENT 				AS "DOCUMENTO",
           C.SMANZANA 					AS "MANZANA",
           C.NLOTE 						AS "LOTE",
           U.SURBANIZACION 				AS "CONDOMINIO",
    ...
    FROM casas C
    INNER JOIN clientes CL ON CL.NID = C.NCLIENTE
    INNER JOIN casas_servicios S ON S.NCASA = C.NID
    INNER JOIN urbanizaciones U ON U.NID = C.NURBANIZACION
    INNER JOIN servicios SR ON SR.NID = S.NSERVICIO
    INNER JOIN detalle_pago DP ON DP.NSERVICIO = S.NID AND DP.NSTATUS = 0
    LEFT JOIN usuario US ON US.NID = DP.NUSER_MEDIDA 
    WHERE C.NURBANIZACION = P_NCONDOMINIO
    AND S.NSERVICIO = P_NSERVICIO
    AND S.NSTATUS = 1;

END$$

CREATE PROCEDURE `SP_SEL_USER` (IN `P_SUSER` VARCHAR(80))   BEGIN
	...
	SELECT DISTINCT U.SUSER    		AS "USUARIO",
		   U.NSTATUS  				AS "ESTADO",
		   U.SNOMBRE 				AS "NOMBRE",
		   U.SAPELLIDO_PAT 			AS "APELLIDO_PAT",
		   U.SAPELLIDO_MAT 			AS "APELLIDO_MAT",
	  ...
	  FROM usuario U
     INNER JOIN usuario_urbanizacion C ON C.NUSUARIO = U.NID 
	 WHERE U.SUSER 	 <> P_SUSER 
	   AND U.SUSER 	 <> "ROOT"
	   AND U.NACTIVO =  1
	   AND C.NURBANIZACION IN (SELECT UU.NURBANIZACION  
	  							 FROM usuario_urbanizacion UU
	  							WHERE UU.NUSUARIO = V_NIDUSER);

 END$$

CREATE PROCEDURE `SP_SEL_USER_DNI` (IN `P_SDOCUMENT` VARCHAR(9))   BEGIN
	...
	SELECT U.SDOCUMENT 			AS "DOCUMENTO", 
	       ...
	       U.NPASSWORD_RESET 	AS "RESET",
	       U.SDIRECCION 		AS "DIRECCION",
	       U.NDEPARTAMENTO 		AS "DEPARTAMENTO",
	       U.NPROVINCIA 		AS "PROVINCIA",
	       U.NDISTRITO 			AS "DISTRITO",
	       UU.NURBANIZACION 	AS "CONDOMINIO"
	  FROM usuario U
	  LEFT JOIN usuario_urbanizacion UU ON UU.NUSUARIO  = U.NID  
	 WHERE U.SDOCUMENT = P_SDOCUMENT;
	
END$$

CREATE PROCEDURE `SP_VAL_CLIENT_DNI` (IN `P_SDOCUMENT` VARCHAR(11))   BEGIN
...
CREATE PROCEDURE `SP_VAL_PARAMETER` (IN `P_SLABEL` VARCHAR(99))   BEGIN
...
CREATE PROCEDURE `SP_VAL_SUMINISTRO` (IN `P_NSUMINISTRO` DECIMAL(11), IN `P_NURBANIZACION` INT, IN `P_NIDUSER` INT)   BEGIN
	
    SELECT C.NSUMINISTRO  AS "ID"
      FROM casas C
     INNER JOIN usuario_urbanizacion U ON U.NURBANIZACION = C.NURBANIZACION 
     WHERE C.NSUMINISTRO   = P_NSUMINISTRO
   	   AND C.NURBANIZACION = P_NURBANIZACION
   	   AND U.NUSUARIO      = P_NIDUSER;
...
CREATE PROCEDURE `SP_VAL_USER` (IN `P_SUSER` VARCHAR(50))   BEGIN
    ...
    SELECT COUNT(*) AS "CANTIDAD"
    FROM usuario
    WHERE SUSER = TRIM(P_SUSER);

END$$

CREATE PROCEDURE `SP_VAL_USER_DOCUMENT` (IN `P_SDOCUMENT` VARCHAR(8))   BEGIN
...
(15, 8, 1, 'AS', 12, '2147483647'),
(16, 9, 1, 'S', 12, '2147483647'),
(17, 8, 2, 'AS', 12, '2147483647'),
(18, 8, 3, 'AS', 12, '2147483647');
...
CREATE TABLE `casas_servicios` (
  ...
  `DINICIO_SERVICIO` date DEFAULT NULL,
...
(17, 15, 1, '2023-05-22', 1, '10.4560'),
(18, 15, 2, NULL, 0, '0.0000'),
(19, 16, 1, '2023-05-23', 1, '0.0000'),
(20, 16, 2, '2023-05-23', 1, '0.0000'),
(21, 17, 1, '2023-05-22', 1, '10.4560'),
(22, 17, 2, NULL, 0, '0.0000'),
(23, 18, 1, '2023-05-22', 1, '10.4560'),
(24, 18, 2, NULL, 0, '0.0000');
...
CREATE TABLE `clientes` (
  ...
  `NDEPARTAMENTO` varchar(2) NOT NULL,
  `NPROVINCIA` varchar(4) NOT NULL,
...
(8, '61293291', 'Marcos', 'gutierrez', 'Huamanttupa', 'ghma28@outlook.com', '910223291', 'Sector 1', '15', '1501', '150142', 1),
(9, '61293292', 'Anker', 'Huamanttupa', 'Gutierrez', 'ghma28@outlook.com', '897898978', 'Sector 1', '15', '1501', '150142', 1);
...
CREATE TABLE `departamento` (
  ...
  `SDEPARTAMENTO` varchar(45) NOT NULL
...
INSERT INTO `departamento` (`NID`, `SDEPARTAMENTO`) VALUES
('01', 'Amazonas'),
('02', 'Áncash'),
('03', 'Apurímac'),
('04', 'Arequipa'),
...
CREATE TABLE `detalle_pago` (
  ...
  `NSUBTOTAL` decimal(8,2) DEFAULT 0.00
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `detalle_pago` (`NID`, `NCLIENTE`, `NSERVICIO`, `NMONTO`, `NDEUDA`, `NTOTAL`, `NUSER`, `NUSER_MEDIDA`, `DREGISTRO`, `SOBSERVACION`, `NMEDICION`, `SIMAGEN`, `NSTATUS`, `DEMISION`, `DVENCIMIENTO`, `NMEDICION_INI`, `NPAGO`, `NMANTENIMIENTO`, `NALUMBRADO`, `NSUBTOTAL`) VALUES
(74, 8, 17, '0.00', '0.00', '0.00', NULL, NULL, NULL, NULL, NULL, NULL, 0, '2023-06-01', '2023-06-07', '10.4560', '0.00', NULL, NULL, '0.00'),
(75, 8, 21, '0.00', '0.00', '0.00', NULL, NULL, NULL, NULL, NULL, NULL, 0, '2023-06-01', '2023-06-07', '10.4560', '0.00', NULL, NULL, '0.00'),
(76, 8, 23, '0.00', '0.00', '0.00', NULL, NULL, NULL, NULL, NULL, NULL, 0, '2023-06-01', '2023-06-07', '10.4560', '0.00', NULL, NULL, '0.00'),
(77, 9, 19, '0.00', '0.00', '0.00', NULL, NULL, NULL, NULL, NULL, NULL, 0, '2023-06-01', '2023-06-07', '0.0000', '0.00', NULL, NULL, '0.00'),
(78, 9, 20, '0.00', '0.00', '0.00', NULL, NULL, NULL, NULL, NULL, NULL, 0, '2023-06-01', '2023-06-07', '0.0000', '0.00', NULL, NULL, '0.00');
...
CREATE TABLE `distrito` (
  `NID` varchar(6) NOT NULL,
  `SDISTRITO` varchar(45) DEFAULT NULL,
  `NPROVINCIA` varchar(4) DEFAULT NULL,
  `NDEPARTAMENTO` varchar(2) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `distrito` (`NID`, `SDISTRITO`, `NPROVINCIA`, `NDEPARTAMENTO`) VALUES
...
(18, 'MEDICION_MIN_LUZ', '36.4', 'N'),
(19, 'PAGO_MIN_LUZ', '120.00', 'N');

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `parametros_servicios_log`
--

CREATE TABLE `parametros_servicios_log` (
  `NID` int(11) NOT NULL,
  `NIDUSER` int(11) DEFAULT NULL,
  `NPARAMETRO` int(11) DEFAULT NULL,
  `SVALUE` varchar(100) DEFAULT NULL,
  `DUPDATE` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Volcado de datos para la tabla `parametros_servicios_log`
--

INSERT INTO `parametros_servicios_log` (`NID`, `NIDUSER`, `NPARAMETRO`, `SVALUE`, `DUPDATE`) VALUES
(1, 1, 16, '6.50', '2023-05-22 18:41:32'),
(2, 1, 16, '7.00', '2023-05-23 20:49:20'),
(3, 1, 16, '8.00', '2023-05-23 20:49:50'),
(4, 1, 16, '5.00', '2023-05-23 20:53:53');
...
CREATE TABLE `provincia` (
  ...
  `SPROVINCIA` varchar(45) NOT NULL,
  `NDEPARTAMENTO` varchar(2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `provincia` (`NID`, `SPROVINCIA`, `NDEPARTAMENTO`) VALUES
('0101', 'Chachapoyas', '01'),
('0102', 'Bagua', '01'),
('0103', 'Bongará', '01'),
...
INSERT INTO `servicios` (`NID`, `SNOMBRE`) VALUES
(1, 'Energía Eléctrica'),
(2, 'Agua potable');
...
(2, 'CONDOMINIO RESIDENCIAL 1', 'logo.jpg', 'Panamerica Sur KM 129.9 - Cerro Azul- Cañete'),
(3, ' CONDOMINIO RESIDENCIAL 2', 'logo.jpg', 'Panamerica Sur KM 129.9 - Cerro Azul- Cañete'),
(4, 'CONDOMINIO RESIDENCIAL 3', 'logo.jpg', 'Panamerica Sur KM 129.9 - Cerro Azul- Cañete');
...
CREATE TABLE `usuario` (
  ...
  `NDEPARTAMENTO` varchar(2) DEFAULT NULL,
  `NDISTRITO` varchar(6) DEFAULT NULL,
  `NPROVINCIA` varchar(4) DEFAULT NULL,
  `NACTIVO` int(1) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `usuario` (`NID`, `NSTATUS`, `SUSER`, `SPASSWORD`, `NPASSWORD_RESET`, `SDOCUMENT`, `SNOMBRE`, `SAPELLIDO_PAT`, `SAPELLIDO_MAT`, `SNUMBER`, `SDIRECCION`, `SEMAIL`, `NDEPARTAMENTO`, `NDISTRITO`, `NPROVINCIA`, `NACTIVO`) VALUES
(1, 1, 'root', '25d55ad283aa400af464c76d713c07ad', 0, '12345678', 'Usuario', 'Prueba', 'Anker', '987654321', 'Villa el salvador', '', NULL, NULL, NULL, 1),
(14, 1, 'anker28', '25d55ad283aa400af464c76d713c07ad', 0, '61293291', 'Anker', 'Gutierrez', 'Huamanttupa', '987654321', 'Villa el salvador', 'ghma28@outlook.com', '15', '150142', '1501', 1),
(15, 1, 'prueba2', '689d8fe00c8671769dc10ff806413983', 0, '61293293', 'Anker', 'Prueba', 'prueba', '987654321', 'prueba villa el salvador', 'ghma28@outlook.com', '15', '150132', '1501', 1),
(16, 1, 'dasdasdas', '4ca2d610c1f6ba2e004dc5ff7a889653', 1, '61293294', 'Anker', 'ADSDASASDDAS', 'DAASDSDADAS', '987654321', 'dfssdffsdfsd', 'asdasd@asd.com', '15', '150714', '1507', 1),
(17, 1, 'PRUEBA3', '049a9e7f9b581fbf5dae1c81c3a09482', 1, '61293296', 'Anker', 'QQQQQQQQQ', 'AAAAA', '987654321', 'Villa el salvador', 'ghma28@outlook.com', '12', '120302', '1203', 1);
...
--
-- Estructura de tabla para la tabla `usuario_urbanizacion`
--

CREATE TABLE `usuario_urbanizacion` (
  `NID` int(11) NOT NULL,
  `NUSUARIO` int(11) DEFAULT NULL,
  `NURBANIZACION` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Volcado de datos para la tabla `usuario_urbanizacion`
--

INSERT INTO `usuario_urbanizacion` (`NID`, `NUSUARIO`, `NURBANIZACION`) VALUES
(1, 1, 1),
(2, 1, 2),
(3, 1, 3),
(4, 1, 4),
...
ALTER TABLE `casas`
  ...
  ADD KEY `NURBANIZACION` (`NURBANIZACION`);
...
ALTER TABLE `casas_servicios`
  ADD PRIMARY KEY (`NID`),
  ADD KEY `casas_servicios_ibfk_1` (`NCASA`),
  ADD KEY `casas_servicios_ibfk_2` (`NSERVICIO`);
...
--
-- Indices de la tabla `parametros_servicios_log`
--
ALTER TABLE `parametros_servicios_log`
  ADD PRIMARY KEY (`NID`);
...
--
-- Indices de la tabla `usuario_urbanizacion`
--
ALTER TABLE `usuario_urbanizacion`
  ADD PRIMARY KEY (`NID`),
  ADD KEY `usuario_urbanizacion_FK` (`NURBANIZACION`),
  ADD KEY `usuario_urbanizacion_FK_1` (`NUSUARIO`);
...
--
ALTER TABLE `detalle_pago`
  MODIFY `NID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=81;
...
--
-- AUTO_INCREMENT de la tabla `parametros_servicios_log`
--
ALTER TABLE `parametros_servicios_log`
...
--
-- AUTO_INCREMENT de la tabla `usuario_urbanizacion`
--
ALTER TABLE `usuario_urbanizacion`
...
--
ALTER TABLE `casas`
  ADD CONSTRAINT `casas_ibfk_1` FOREIGN KEY (`NCLIENTE`) REFERENCES `clientes` (`NID`),
  ADD CONSTRAINT `casas_ibfk_3` FOREIGN KEY (`NURBANIZACION`) REFERENCES `urbanizaciones` (`NID`);
...
--
-- Filtros para la tabla `usuario_urbanizacion`
--
ALTER TABLE `usuario_urbanizacion`
  ADD CONSTRAINT `usuario_urbanizacion_FK` FOREIGN KEY (`NURBANIZACION`) REFERENCES `urbanizaciones` (`NID`),
  ADD CONSTRAINT `usuario_urbanizacion_FK_1` FOREIGN KEY (`NUSUARIO`) REFERENCES `usuario` (`NID`);
COMMIT;
